...
export default function OrderForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://formspree.io/f/mgoykzdp', {
        method: 'POST',
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 sm:gap-6">
          <div className="grid sm:grid-cols-2 gap-4 sm:gap-6">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-cream border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/50"
          ...
          <div>
            ...
            <textarea
              ...
              onChange={handleChange}
              className="w-full px-4 py-3 bg-cream border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/50 resize-none"
              placeholder="איזו מילה/עיצוב תרצו? למי המתנה?"
            />
          </div>
          ...
          {status === 'error' && (
            <div className="flex items-center gap-3 p-4 bg-red-50 rounded-xl text-red-600">
              ...
              <span className="text-sm">משהו השתבש. נסו שוב.</span>
            </div>
          )}
          ...
          <button
            ...
            className="w-full py-4 bg-gradient-to-r from-primary to-gold text-white rounded-xl font-bold text-lg shadow-lg hover:shadow-xl transition-all disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {status === 'loading' ? (
